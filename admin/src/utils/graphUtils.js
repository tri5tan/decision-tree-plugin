 ...
 */
export function getNodesInFirstNLevels(nodes, edges, rootId, levels) {
 ...
 */
function getSubtreeIds(startId, childrenMap) {
  const visited = new Set();
  const queue = [startId];
  while (queue.length) {
 ...
 */
function enforceYesLeftNoRight(nodes, edges) {
  ...
  sortedSources.forEach((sourceId) => {
    ...
    const yesEdge = outEdges.find((e) => e.data?.answer === "Yes");
    const noEdge  = outEdges.find((e) => e.data?.answer === "No");
 ...
 */
function enforceSingleChildCentered(nodes, edges) {
  const posMap = new Map(nodes.map((n) => [n.id, n.position]));
  const childrenMap = buildChildrenMap(edges);
...
export function buildFlowNodes(apiNodes, nodeType = "kb-node") {
...
export function buildFlowEdges(apiEdges) {
  return apiEdges.map((e) => ({
    id: e.id,
    source: e.source,
    target: e.target,
    type: "decision-edge", // maps to DecisionEdge via edgeTypes in TreeEditor
...
export function recomputeNodeStatuses(nodes, edges) {
...
export function computeReachability(nodes, edges, rootNodeId) {
