...
// FD - Flow Diagram colours are defined as CSS variables in Core Framework with fallbacks
//      in the FD object, then composed into contexts below, i.e. STATUS_COLORS, EDGE_COLORS,
//      CHROME, BADGE, etc. This allows both the editor and viewer to share a single source of
//      truth for all colours, and for those colours to be easily overridden by the host site
...
export const FD = {
  ...
  cardBorderSubtle:
    "var(--fd-card-border-subtle, var(--border-primary, #e5e7eb))",
  cardShadow:
    "var(--fd-card-shadow, var(--shadow-primary, 0 1px 4px rgba(0,0,0,0.08)))",
  ...
  modalShadow:
    "var(--fd-modal-shadow, var(--shadow-primary, 0 8px 32px rgba(0,0,0,0.25)))",
...
export const STATUS_COLORS = {
  start: {
    base: FD.statusStartBase,
    bg: FD.statusStartBg,
    border: FD.statusStartBorder,
    text: FD.statusStartText,
  },
  complete: {
    base: FD.statusCompleteBase,
    bg: FD.statusCompleteBg,
    border: FD.statusCompleteBorder,
    text: FD.statusCompleteText,
  },
  partial: {
    base: FD.statusPartialBase,
    bg: FD.statusPartialBg,
    border: FD.statusPartialBorder,
    text: FD.statusPartialText,
  },
  empty: {
    base: FD.statusEmptyBase,
    bg: FD.statusEmptyBg,
    border: FD.statusEmptyBorder,
    text: FD.statusEmptyText,
  },
  terminal: {
    base: FD.statusTerminalBase,
    bg: FD.statusTerminalBg,
    border: FD.statusTerminalBorder,
    text: FD.statusTerminalText,
  },
  orphan: {
    base: FD.statusOrphanBase,
    bg: FD.statusOrphanBg,
    border: FD.statusOrphanBorder,
    text: FD.statusOrphanText,
  },
};
...
export function getNodeMinimapColor(data?: {
  isOrphan?: boolean;
  isRoot?: boolean;
  linkStatus?: string;
}): string {
  const key = data?.isOrphan
    ? "orphan"
    : data?.isRoot
      ? "start"
      : data?.linkStatus;
...
export const STATUS_META = {
  start: {
    icon: "▶",
    label: STATUS_LABELS.start,
    color: STATUS_COLORS.start.text,
    bg: STATUS_COLORS.start.bg,
  },
  complete: {
    icon: "✓",
    label: STATUS_LABELS.complete,
    color: STATUS_COLORS.complete.text,
    bg: STATUS_COLORS.complete.bg,
  },
  partial: {
    icon: "◑",
    label: STATUS_LABELS.partial,
    color: STATUS_COLORS.partial.text,
    bg: STATUS_COLORS.partial.bg,
  },
  empty: {
    icon: "○",
    label: STATUS_LABELS.empty,
    color: STATUS_COLORS.empty.text,
    bg: STATUS_COLORS.empty.bg,
  },
  terminal: {
    icon: "■",
    label: STATUS_LABELS.terminal,
    color: STATUS_COLORS.terminal.text,
    bg: STATUS_COLORS.terminal.bg,
  },
  orphan: {
    icon: "⚠",
    label: STATUS_LABELS.orphan,
    color: STATUS_COLORS.orphan.text,
    bg: STATUS_COLORS.orphan.bg,
  },
};
